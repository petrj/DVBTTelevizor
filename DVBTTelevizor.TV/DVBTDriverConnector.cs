...
namespace DVBTTelevizor
{
    public class DVBTDriverConnector : IDriverConnector
    {
        ...
        private async Task<DVBTDriverResponse> SendRequest(DVBTDriverRequest request, int secondsTimeout = 20)
        {
            ...
            return await Task.Run(async () =>
            {
                ...
                catch (TimeoutException)
                {
                    ...
                    return new DVBTDriverResponse() { SuccessFlag = false };
                }
                catch (Exception ex)
                {
                ...
                var response = new DVBTDriverResponse()
                {
                    SuccessFlag = true,
        ...
        public async Task<DVBTDriverStatus> GetStatus()
        {
            ...
            var status = new DVBTDriverStatus();
            ...
            */
            ...
            var req = new DVBTDriverRequest(DVBTDriverRequestTypeEnum.REQ_GET_STATUS, new List<long>(), responseSize);
            ...
            {
                StatusChanged(this, new DVBTDriverStatusChangedEventArgs() { Status = status });
            }
        ...
        public async Task<DVBTDriverVersion> GetVersion()
        {
            ...
            var version = new DVBTDriverVersion();

            var responseSize = 26;  // 1 + 1 + 8 + 8 + 8

            var req = new DVBTDriverRequest(DVBTDriverRequestTypeEnum.REQ_PROTOCOL_VERSION, new List<long>(), responseSize);
            ...
            version.SuccessFlag = DVBTDriverStatus.GetBigEndianLongFromByteArray(ar, 2) == 1;
            version.Version = DVBTDriverStatus.GetBigEndianLongFromByteArray(ar, 10);
            version.AllRequestsLength = DVBTDriverStatus.GetBigEndianLongFromByteArray(ar, 18);
        ...
        public async Task<DVBTDriverResponse> Tune(long frequency, long bandwidth, int deliverySystem)
        {
            ...
            try
            {
                ...
                var req = new DVBTDriverRequest(DVBTDriverRequestTypeEnum.REQ_TUNE, payload, responseSize);
                var response = await SendRequest(req, 10);

                if (response.Bytes.Count < responseSize)
                    throw new Exception($"Bad response, expected {responseSize} bytes, received {response.Bytes.Count}");

                var successFlag = DVBTDriverStatus.GetBigEndianLongFromByteArray(response.Bytes.ToArray(), 2);
        ...
        private async Task<DVBTDriverResponse> SendCloseConnection()
        {
            _log.Debug($"Closing connection");

            var responseSize = 10;

            var req = new DVBTDriverRequest(DVBTDriverRequestTypeEnum.REQ_EXIT, new List<long>(), responseSize);
            var response = await SendRequest(req, 5);
        ...
        public async Task<DVBTDriverResponse> SetPIDs(List<long> PIDs)
        {
            try
            {
                ...
                var req = new DVBTDriverRequest(DVBTDriverRequestTypeEnum.REQ_SET_PIDS, PIDs, responseSize);
                var response = await SendRequest(req, 5);
        ...
        public async Task<DVBTDriverCapabilities> GetCapabalities()
        {
            ...
            var cap = new DVBTDriverCapabilities();

            var responseSize = 2 + 7 * 8;

            var req = new DVBTDriverRequest(DVBTDriverRequestTypeEnum.REQ_GET_CAPABILITIES, new List<long>(), responseSize);
            var response = await SendRequest(req, 5);
        ...
        public async Task<DVBTDriverSearchAllPIDsResult> SearchProgramPIDs(List<long> MapPIDs)
        {
            ...
            var res = new DVBTDriverSearchAllPIDsResult();
        ...
        public async Task<DVBTDriverSearchPIDsResult> SearchProgramPIDs(long mapPID, bool setPIDsAndSync)
        {
            ...
            var res = new DVBTDriverSearchPIDsResult();
        ...
        public async Task<DVBTDriverTuneResult> TuneEnhanced(long frequency, long bandWidth, int deliverySystem, bool fastTuning)
        {
            ...
            try
            {
                for (var j = 1; j < tuneAttemptsCount; j++)
                {
                    ...
                    if (res.Result != DVBTDriverSearchProgramResultEnum.OK)
                    {
                        return res;
                    }
        ...
        public async Task<DVBTDriverSearchPIDsResult> SetupChannelPIDs(long mapPID, bool fastTuning)
        {
            ...
            try
            {
                ...
                if (pmtTableSearchRes.Result != DVBTDriverSearchProgramResultEnum.OK)
                {
        ...
        public async Task<DVBTDriverSearchProgramMapPIDsResult> SearchProgramMapPIDs(bool tunePID0and17 = true)
        {
            ...
            var res = new DVBTDriverSearchProgramMapPIDsResult();
